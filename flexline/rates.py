...
# Ages 56–69 marked * in the product guide: available only under Recognition of Prior Coverage.
CI_RATES: Dict[int, float] = {
...
def get_life_rate(age: int) -> float:
    return LIFE_RATES.get(age, 0.0)


def get_ci_rate(age: int) -> float:
    return CI_RATES.get(age, 0.0)
...
PROVINCES: List[str] = sorted(PROVINCIAL_TAX_RATES.keys())
